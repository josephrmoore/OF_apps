...
//------------------------------------------------------------------
void demoParticle::changeScale(float newscale, int r1, int g1, int b1, int r2, int g2, int b2, int r3, int g3, int b3){
    
	if( mode == PARTICLE_MODE_ATTRACT ){
		ofSetColor(r1*scale, g1*scale, b1*scale);
	}
	else if( mode == PARTICLE_MODE_REPEL ){
		ofSetColor(r3*scale, g3*scale, b3*scale);
	}
	else if( mode == PARTICLE_MODE_NOISE ){
		ofSetColor(ofRandom(100,255));
	}
	else if( mode == PARTICLE_MODE_NEAREST_POINTS ){
		ofSetColor(r2*scale, g2*scale, b2*scale);
	}
    
	ofCircle(pos.x, pos.y, scale * newscale * 4.0);
}
